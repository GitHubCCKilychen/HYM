...
+ (BOOL)xfunc_isMobileNumber1:(NSString*)mobileNum
{
...
//获取剩余时间的文本
+(NSString*)getStringTime:(int)second
{
    if (second/60 == 0)// 不足1分钟时
    {
        // return [NSString stringWithFormat:@"00:00:%@",[self getNewTime:second]];
        return [NSString stringWithFormat:@"00:%@",[self getNewTime:second]];
    }
    else if(second/3600 == 0)// 不足1小时
    {
        // return [NSString stringWithFormat:@"00:%@:%@",[self getNewTime:second/60],[self getNewTime:second%60]];
        return [NSString stringWithFormat:@"%@:%@",[self getNewTime:second/60],[self getNewTime:second%60]];
    }
    return @"00:00";
//    else
//    {
//        return [NSString stringWithFormat:@"%@:%@:%@",[self getNewTime:second/3600], [self getNewTime:(second%3600)/60],[self getNewTime:second%60]];
//    }
}

//数字前加个0
+(NSString*)getNewTime:(int)second
{
    if (second>9)
    {
...
+(UIImage*)getMapImageByUrl:(NSString*)imgurl
{
    ...
    UIImage *image = [UIImage imageNamed:@"新地点透明.png"];
    ...
    //绘制图片  con:绘制图片的上下文  CGRectMake:图片的原点和大小  imageLeft:当前的CGImage
    NSLog(@"长度：%f,%f,%f,%f",szcore.width,szcore.height,sz.width,sz.height);
    CGContextDrawImage(con, CGRectMake(0, 28, sz.width, sz.width), imageRight);
    CGContextDrawImage(con, CGRectMake(0, 0, sz.width, sz.height), imageLeft);
...
+ (NSString *)encodePassword:(NSString *)password
{
    ...
    // datekey+加密后的密码
    NSString * md5Str = [NSString stringWithFormat:@"%@%@", DATAKEY, md5Password];
    // 再加密
